...
// Include generated Qt Protobuf header
#include "rm_protocol.qpb.h"

class RMClient : public QObject
{
    Q_OBJECT
public:
    explicit RMClient(QObject *parent = nullptr);
    void connectToHost();

    // --- Control Methods (Proxy to RMSender) ---
    // Manual send command for remote control
...
signals:
    // Connection State
    void sigConnectionStateChanged(i32 state);

    // Log signal for UI
    void sigPacketLog(LogLevel level, bool isSend, const QString &topic, const QString &details);

    // --- Decoupled Signals for UI Updates (Proxy from RMReceiver) ---
    // 2.2.2 GameStatus
